...
export async function fetchDerivativeToken(address: string): Promise<Derivative> {
    ...
    for (const underlyingTokenAddress of lpTokenDetails.underlyings) {
        ...
        underlyingTokens.push(new TokenBalance(new Erc20(erc20Details.address, erc20Details.symbol, erc20Details.name, erc20Details.decimals, price_token_usd, erc20Details.logo), user_balance));
    }
    ...
    const lpToken = new Derivative(lpTokenDetails.address, lpTokenDetails.symbol, lpTokenDetails.name, lpTokenDetails.decimals, protocol, logo, underlyingTokens, totalSupply, APY, TVL, lpTokenDetails.type);
    return lpToken;
}
...
export async function fetchToken(address: string): Promise<Erc20> {
    ...
    const token = new Erc20(tokenDetails.address, tokenDetails.symbol, tokenDetails.name, tokenDetails.decimals, price, logo);
    return token;
}
