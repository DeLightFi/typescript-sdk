...
export class Derivative extends Erc20 {
    ...
    constructor(
        ...
        const price = (underlyings.reduce((accumulator: Decimal, currentValue: TokenBalance): Decimal => {
            const tokenValue = currentValue.balance.times(currentValue.token.price).div(new Decimal(10).pow(currentValue.token.decimals));
            return accumulator.plus(tokenValue);
        }, new Decimal(0))).div(decimals.div(new Decimal(10).pow(decimals)));
